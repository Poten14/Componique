...
const Profile = () => {
    const { shape, size, color, variant, text, iconSize, iconColor, iconPosition, nickname, placeholder, job, location, email, } = useProfileStore();
    return (<div className="grid place-items-center gap-y-6">
      <div className="grid place-items-center gap-y-6 rounded-lg bg-white p-8 shadow-lg dark:bg-[#333742]">
        <ImageUpload shape={shape} size={size} color={color} variant={variant} text={text} icon="icon-user" iconSize={iconSize} iconColor={iconColor} iconPosition={iconPosition}/>
        <SelectList option={["65 Post", "65 Follower", "65 Follow "]} className="rounded-lg bg-Basic text-white"/>
