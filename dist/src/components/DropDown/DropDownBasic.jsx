...
const DropDownBasic = ({ option, placeholder = "Select Option", onSelect, type = "basic", // 기본 스타일은 "basic"
    ...
    return (<div className={`relative inline-block w-48 ${getClassNames()}`}>
      ...
      {isOpen && (<ul className="absolute z-10 w-full bg-white dark:bg-[#2A2E39]">
          {option.map((item, index) => (<li key={index} onClick={() => handleOptionClick(item)} className="cursor-pointer p-2 text-zinc-800 hover:bg-[#E8F5FF] dark:text-white dark:hover:bg-[#2A6490]">
              {item}
            </li>))}
        </ul>)}
    </div>);
