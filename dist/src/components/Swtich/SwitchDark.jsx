    ...
    return (<div className="mr-3 flex items-center justify-center space-x-12">
      <div onClick={toggleSwitch} className={`flex h-10 w-14 cursor-pointer items-center rounded-full p-1 shadow-lg ${isDarkMode ? "bg-[#232A31]" : "bg-[#FFFFFF]"}`}>
        ...
        <div className="relative flex h-full items-center">
          {/* Sun 아이콘 */}
          <FaMoon className={`absolute left-1 text-[#EDCE7B] transition-opacity duration-500 ease-in-out ${isDarkMode ? "opacity-100" : "opacity-0"}`}/>
          {/* Moon 아이콘 */}
          <BsSunFill className={`absolute translate-x-7 text-[#FFECB8] transition-opacity duration-500 ease-in-out ${isDarkMode ? "opacity-0" : "opacity-100"}`}/>
          {/* 스위치 버튼 */}
