...
completed = false, // completed 기본값을 false로 설정
    ...
    return (<div className="relative flex h-32 w-32 items-center justify-center">
      <div className="flex h-full w-full items-center justify-center rounded-full" style={{
        ...
        <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white">
          ...
          {showPercent && (<span className="text-lg font-medium text-Basic">
              {`${progressBar}%`}
            </span>)}
          {/* 진행률이 100%이고 completed가 true이면 체크 표시를 보여줌 */}
          {completed && progressBar === 100 && (<span className="text-lg font-medium text-Basic">✅</span>)}
        </div>
      </div>
    </div>);
};
