...
const Toast = ({ children, isOpen = true, onClose, color = "basic", size = "medium", position = "leftBottom", text = "left", variant = "solid", isClose = false, isProgress = false, className, time, path, ...rest }) => {
    ...
    return (<>
      <section className={`${BasicToast} ${ToastPosition[position]} ${ToastSize[size]} ${isToastOpen ? "opacity-100" : "pointer-events-none opacity-0"} ${path ? "cursor-pointer" : ""} ${className || ""} z-[999]`} onClick={() => {
        ...
        <div className={`flex items-center justify-between rounded-md ${ToastVariant} p-4`} {...rest}>
          <div className={`${ToastTextAlign[text]} `}>{children}</div>

          <button className={`z-999 group relative ml-2 h-4 w-4 bg-transparent ${isClose ? "hidden" : "block"}`} onClick={onclickCloseHandler}>
            <span className={`absolute left-1/2 top-1/2 block h-0.5 w-full -translate-x-1/2 -translate-y-1/2 rotate-45 transform ${closeButtonColor}`}></span>
            <span className={`absolute left-1/2 top-1/2 block h-0.5 w-full -translate-x-1/2 -translate-y-1/2 -rotate-45 transform ${closeButtonColor}`}></span>
          </button>
        </div>
        {isProgress && (<div className="bg-gray-200 relative h-1 w-full rounded">
            {/* div의 너비는 progress 상태에 의해 동적으로 설정됨, 처음에는 100%로 시작하고 시간이 지나면서 점차 줄어들게 됨 */}
            <div className={`absolute left-0 h-full ${progressBarColor} rounded ${variant === "solid" ? "top-[-4px]" : "top-[-6px]"}`} style={{ width: `${progress}%` }}/>
          </div>)}
      </section>
    </>);
};
