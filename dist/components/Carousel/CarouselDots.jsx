      ...
      <div className="relative w-full">
        <div className="relative m-auto h-96 overflow-hidden rounded-lg">
          ...
          {showDots && (<div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 transform space-x-2">
              {images.map((_, dots) => (<button key={dots} className={`h-3 w-3 rounded-full ${dots === currentSlide ? "bg-white" : "bg-gray"}`} onClick={() => handleDotClick(dots)}/>))}
            </div>)}
          <button type="button" className="group absolute start-0 top-0 flex h-full cursor-pointer items-center justify-center px-4 focus:outline-none" onClick={handlePrev}>
          ...
          <button type="button" className="group absolute end-0 top-0 flex h-full cursor-pointer items-center justify-center px-4 focus:outline-none" onClick={handleNext}>
