...
const Drawer = ({ menu, logo, isOpen = true, color, bgColor = "basic", onClose, className, position = "left", }) => {
    ...
    return (<section className={`fixed left-0 top-0 z-[999] h-full w-full select-none !bg-black !bg-opacity-50 transition-opacity duration-500 ease-in-out ${className || ""} ${isDrawerOpen ? "opacity-100" : "pointer-events-none opacity-0"}`} onClick={onClickBackgroundHandler}>
      <div className={`${basicBg} ${motion}`} 
    //검은배경 말고 안에 메뉴 클릭시 전파 방지
    onClick={(e) => e.stopPropagation()}>
        <div className={`absolute ${position === "right"
            ? "left-[-40px]"
            : position === "left"
                ? "right-[-40px]"
                : position === "top"
                    ? "bottom-[-40px] right-2"
                    : "right-2 top-[-40px]"}`}>
          <button className="group relative h-8 w-8 bg-transparent" onClick={onclickCloseHandler}>
        ...
        <div className={`flex justify-center py-2 ${position === "top" || position === "bottom" ? "my-2" : "my-0"}`} onClick={onClickLogoCloseHandler}>
          <img src={logo} className={`cursor-pointer text-center ${position === "top" || position === "bottom" ? "w-[300px]" : "w-[180px]"}`}/>
        </div>
        <div className="h-[calc(100%-90px)]">
          <ul className={`${position === "top"
            ? "flex flex-wrap justify-center pb-2"
            : position === "bottom"
                ? "space-y-2 pb-4"
                : "space-y-2"} h-full overflow-y-auto`}>
            {menu?.map((group, index) => (<li key={index} className={`mb-4 ${position === "top" || position === "bottom" ? "w-full" : ""}`}>
                {group.groupName && (<div className={`mx-2 mb-2 rounded px-2 font-semibold text-white ${group.groupNameClassName || ""}`}>
                    {group.groupName}
                  </div>)}
                <ul className="space-y-2">
                  {group.items?.map((item, itemIndex) => (<li key={itemIndex} className={`${position === "top" || position === "bottom"
