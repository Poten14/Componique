  ...
  const templateList: templateListType[] = [
    {
      ...
      description:
        "사용자 아이디, 문구, 이미지 등 원하는 내용을 적고 소개 할 수 있는 폼입니다.",
    },
    {
      title: "Profile",
      path: "/profile",
    ...
    },
    {
      ...
      description:
        "사용자가 원하는 색상을 선택하거나 만들어서 배경에 적용해볼 수 있는 페이지입니다. ",
    },
    {
      title: "Shopping",
      path: "/shopping",
      ...
      <div className="flex h-full flex-wrap justify-center overflow-y-auto rounded-2xl">
        <ul className="mt-10 flex w-full flex-wrap justify-evenly">
          {templateList.map((item, index) => (
            <li
              key={index}
              className="relative mb-5 cursor-pointer rounded-lg"
            ...
            >
              ...
              />
              <article
                className={`text-md absolute bottom-0 left-0 z-0 w-[200px] overflow-hidden rounded-b-xl bg-gradient-to-b from-transparent to-blue-400/60 p-3 transition-transform duration-1000 ease-out dark:from-transparent dark:to-white/60 ${
              ...
              >
                ...
                <div className="mt-2 flex w-full flex-wrap justify-center text-center">
                  {item.component.map((component: any, componentIndex: any) => (
                    <p
                      key={componentIndex}
                      className="m-1 rounded-full border border-Basic p-1 px-2 text-sm font-bold text-white dark:border-Navy dark:text-Navy"
                    >
                      {component}
                    </p>
                  ))}
